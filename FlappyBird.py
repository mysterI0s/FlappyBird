...
while run:
	...
	if len(pipe_group) > 0:
		...
		if pass_pipe == True:
			if bird_group.sprites()[0].rect.left > pipe_group.sprites()[0].rect.right:
				score += 1
				pass_pipe = False
	draw_text(str(score), font, white, int(screen_width / 2), 80)
	...
	if game_over == False and start_the_game == True:
	...
	for event in pygame.event.get():
		if event.type == pygame.QUIT:
			run = False
		if event.type == pygame.KEYDOWN and start_the_game == False and game_over == False:
			start_the_game = True
